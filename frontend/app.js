...
function handleServerEvent(type, data) {
  switch (type) {
    ...
    case 'user_question':
      // Phase 6a — agent paused mid-task to ask the user a question
      showQuestionModal(data.question_id, data.question);
      setStatusBar('Agent is waiting for your answer…', 'active');
      break;
...
// ============================================================
// Question modal (Phase 6a) — agent pauses mid-task to ask user
// ============================================================

const questionModal  = document.getElementById('question-modal');
const questionText   = document.getElementById('question-text');
const questionInput  = document.getElementById('question-input');
const btnQSubmit     = document.getElementById('btn-question-submit');

/**
 * Show the question modal and suspend until the user submits an answer.
 * The answer is sent back to the server as a question_answer message so
 * the TaskRunner's ask_user() coroutine can resume.
 *
 * @param {string} questionId  - UUID generated by the ask_user tool
 * @param {string} question    - Question text to display
 */
function showQuestionModal(questionId, question) {
  ...
  // Replace any previous click handler to bind the current questionId
  btnQSubmit.onclick = () => {
    ...
    // Echo a short confirmation in the chat so there is a visible record
    appendStatusMessage(
      `You answered: "${answer.slice(0, 80)}${answer.length > 80 ? '…' : ''}"`
    );
    setStatusBar('Answer sent — agent continuing…', 'active');
  };
}

// Allow Ctrl+Enter to submit the question modal
...
// Clicking the backdrop dismisses nothing — the user must answer (or wait for timeout)
// We intentionally do NOT add a click-outside handler for the question modal.
