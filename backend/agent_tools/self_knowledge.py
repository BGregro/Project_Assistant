...
def register_self_knowledge_tools() -> None:
    ...
    register_tool(
        ...
        description=(
            "Phase 12b: Analyze agent performance using performance_metrics.json "
            "(falls back to long_term.json for pre-12b data). "
            "Reads per-tool call counts, success rates, duration averages, and "
            "per-task-type statistics, then generates 3-5 actionable improvement "
            "suggestions via the local LLM at no Claude API cost. "
            "Returns: tool_metrics, task_type_metrics, top_failing_tools, "
            "improvement_suggestions, and data_source. "
    ...
    register_tool(
        ...
        description=(
            "Phase 12b: Return live per-tool and per-task-type performance metrics "
            "from performance_metrics.json. "
            "Each tool entry includes: call_count, success_count, failure_count, "
            "avg_duration_ms, success_rate_pct, failure_rate_pct, last_failure_reason. "
            "Without a tool_name argument, returns all tools + task_type breakdown + "
            "top_failing_tools. Pass tool_name to get stats for one specific tool. "
            "Non-destructive — read-only access to the metrics database."
        ),
        input_schema={
            "type": "object",
            "properties": {
                "tool_name": {
                    "type": "string",
                    "description": "Optional: name of a specific tool to look up. Leave empty for all tools.",
                }
            },
            "required": [],
        },
...
async def analyze_performance() -> dict:
    ...
    Reads per-tool call counts, success rates, and duration averages, plus
    per-task-type statistics. Generates 3-5 actionable improvement suggestions
    via the local LLM (qwen3:14b) at no Claude API cost.

    Falls back to the long_term.json task history when performance_metrics.json
    is empty or missing (e.g. on a fresh install before any tasks have run).
    """
    from .local_llm import local_llm_call

    # ── Try Phase 12b metrics first ────────────────────────────────────
    perf_module = None
    tools: dict = {}
    task_types: dict = {}
    try:
        from memory import performance as perf_module
        metrics = perf_module.load()
        tools = metrics.get("tools", {})
        task_types = metrics.get("task_types", {})
    except Exception as _pm_e:
        logger.warning(f"[self_knowledge] Could not load performance metrics (non-fatal): {_pm_e}")

    if tools and perf_module is not None:
        # ── Phase 12b path — real metrics available ────────────────────
        top_failing = perf_module.get_top_failing_tools(5)
        all_tool_stats = perf_module.get_tool_metrics()
        task_type_metrics = perf_module.get_task_type_metrics()

        summary_lines = [f"Tool metrics ({len(tools)} tools tracked):"]
        if top_failing:
            summary_lines.append(
                "Top failing tools: "
                + str([(t["tool"], f"{t['failure_rate_pct']}%") for t in top_failing])
            )
        if task_types:
            summary_lines.append(
                "Task types: "
                + str([
                    (k, f"{v.get('success_count', 0)}/{v.get('count', 0)} success")
        ...
        prompt = (
            f"An AI agent has these performance metrics:\n{summary}\n\n"
            "Write 3-5 specific, actionable improvement suggestions. "
            "Focus on the tools with highest failure rates and task types with lowest success rates. "
            "Be concrete — suggest code changes, retry logic, or alternative approaches. "
            "Format as a numbered list."
        )
        logger.info("[self_knowledge] Requesting improvement suggestions from local LLM (Phase 12b)…")
        suggestions = await local_llm_call(prompt, "qwen3:14b", base_url="http://localhost:11434")
        if not suggestions:
            suggestions = "Could not generate suggestions — local LLM unavailable."

        return {
            "success": True,
            "tool_metrics": all_tool_stats,
            "task_type_metrics": task_type_metrics,
            "top_failing_tools": top_failing,
            "improvement_suggestions": suggestions,
            "data_source": "performance_metrics.json",
        }

    # ── Fallback: derive from long_term.json (pre-Phase-12b data) ─────
    logger.info("[self_knowledge] performance_metrics.json empty — falling back to long_term.json")
    ...
    if len(tasks) < 5:
        return {
            ...
            "data_source": "long_term.json (fallback)",
        }
    ...
    logger.info("[self_knowledge] Requesting improvement suggestions from local LLM (fallback)…")
    suggestions = await local_llm_call(prompt, "qwen3:14b", base_url="http://localhost:11434")
    if not suggestions:
        suggestions = "Could not generate suggestions — local LLM unavailable."
...
# ---------------------------------------------------------------------------
# Tool: get_performance_metrics  (Phase 12b)
# ---------------------------------------------------------------------------

async def get_performance_metrics(tool_name: str = "") -> dict:
    """
    Return live performance metrics from performance_metrics.json.

    Args:
        tool_name: If provided, return stats for that specific tool only.
                   If empty, return all tools sorted by call count descending,
                   a summary block, per-task-type stats, and top failing tools.

    Each tool entry includes computed fields:
        success_rate_pct, failure_rate_pct, avg_duration_ms.
    """
    try:
        from memory import performance as perf_module
        if tool_name:
            result = perf_module.get_tool_metrics(tool_name)
        else:
            result = {
                **perf_module.get_tool_metrics(),
                "task_types": perf_module.get_task_type_metrics(),
                "top_failing_tools": perf_module.get_top_failing_tools(5),
            }
        return {"success": True, **result}
    except Exception as e:
        logger.warning(f"[self_knowledge] get_performance_metrics error: {e}")
